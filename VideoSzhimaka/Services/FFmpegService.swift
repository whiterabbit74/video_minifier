...
class FFmpegService: FFmpegServiceProtocol, @unchecked Sendable {
    ...
    func cancelCurrentOperation() {
        ...
        processQueue.async { [weak self] in
            ...
            // Сохраняем ссылки перед очисткой с блокировкой
            var processToTerminate: Process?
            var progressTaskToCancel: Task<Void, Never>?

            self.lock.lock()
            processToTerminate = self.currentProcess
            progressTaskToCancel = self.currentProgressTask
        ...
        do {
            try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
                ...
                do {
                    ...
                    self.lock.lock()
                    let isCancelledImmediate = self.currentProcess == nil
                    self.lock.unlock()
            ...
            self.lock.lock()
            let isCancelledFinal = self.currentProcess == nil
            self.lock.unlock()

            guard !isCancelledFinal else {
